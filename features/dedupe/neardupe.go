...
func FindNearDuplicates(photos []PhotoFile, threshold int) map[string][]PhotoFile {
	...
	for i := range photos {
		...
		for j := i + 1; j < len(photos); j++ {
			if !isImageFile(photos[j].Path) || photos[j].Dhash == 0 {
				continue
			}
...
// SelectBestCandidate selects the best image from a group of near duplicates
// Priority:
